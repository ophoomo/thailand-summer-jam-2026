...
struct HealthComp {
    ...
    [[nodiscard]] bool  isDead()  const noexcept { return current <= 0; }
...
struct StrengthComp  { int32_t value{0}; };  // +N to all attack damage
struct DexterityComp { int32_t value{0}; };  // +N to all block gained
...
struct HandComp {
    ...
    HandComp() { slots.fill(-1); }

    [[nodiscard]] bool isFull()  const noexcept { return count >= HAND_CAPACITY; }
    ...
    void removeAt(int32_t slot_index) noexcept {
        if (slot_index < 0 || slot_index >= count) return;
...
struct EnemyTag    {};
struct BossTag     {};
struct DeadTag     {};          // marked for removal on next resolve pass
struct ActiveTurnTag {};        // entity currently acting
