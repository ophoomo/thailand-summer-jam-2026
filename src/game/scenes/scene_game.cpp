        ...
        if (auto *sprite = reg.try_get<battle::SpriteComp>(e)) {
            if (!sprite->texture_id.empty()) {
                ...
                if (auto *anim = reg.try_get<battle::AnimComp>(e)) {
                    ...
                    // Default idle animation: use frame_count from component or default
                    frame = static_cast<int>(this->m_enemy_anim_time / 0.1f) % frame_count;
                }
...
void SceneGame::onEnemyAttack(const battle::EvEnemyAttack &event)
{
    ...
    anim.elapsed = 0.0f;
    anim.frame = 0;
    anim.frame_count = 4;  // attack animation has 4 frames
...
void SceneGame::onEnemySpawned(const battle::EvEnemySpawned &event)
{
    ...
    anim.elapsed = 0.0f;
    anim.frame = 0;
    anim.frame_count = 4;  // enter animation has 4 frames
    anim.is_playing = true;
    anim.loop = false;
}
...
void SceneGame::onEnemyDied(const battle::EvEnemyDied &event)
{
    ...
    if (reg.valid(event.enemy)) {
        ...
        anim.elapsed = 0.0f;
        anim.frame = 0;
        anim.frame_count = 4;  // die animation has 4 frames
        anim.is_playing = true;
        anim.loop = false;
    }
}
