...
BattleSystem::BattleSystem(entt::dispatcher& dispatcher,
...
{
    return m_ctx.phase != CombatPhase::IDLE
        && m_ctx.phase != CombatPhase::VICTORY
...
void BattleSystem::buildStarterDeck(DeckComp& deck) const
{
    auto push = [&](int id, int count = 1) {
        if (m_card_db.findByIndex(id))
            for (int i = 0; i < count; ++i) deck.draw_pile.push_back(id);
        else
...
{
    if (m_ctx.phase == CombatPhase::IDLE) return;
...
{
    switch (m_ctx.phase) {
    case CombatPhase::SETUP:            transitionTo(CombatPhase::PLAYER_DRAW); break;
    case CombatPhase::PLAYER_DRAW:      phasePlayerDraw();        break;
    case CombatPhase::PLAYER_TURN:      phasePlayerTurn(dt);      break;
    case CombatPhase::ENEMY_TURN:       phaseEnemyTurn(dt);       break;
    case CombatPhase::RESOLVE_DEATHS:   phaseResolveDeaths();     break;
    case CombatPhase::LEVEL_TRANSITION: phaseLevelTransition(dt); break;
    case CombatPhase::VICTORY:          phaseVictory();           break;
    case CombatPhase::DEFEAT:           phaseDefeat();            break;
...
{
    LOG_TRACE("[BattleSystem] Phase {} → {}",
              static_cast<int>(m_ctx.phase), static_cast<int>(next));
    m_ctx.phase       = next;
...
{
    if (m_ctx.phase != CombatPhase::PLAYER_TURN) return;

    auto& hand   = m_reg.get<HandComp>(m_player);
    auto& energy = m_reg.get<EnergyComp>(m_player);

    if (hand_slot < 0 || hand_slot >= hand.count) return;
    ...
    if (energy.current < fx->cost) {
        LOG_TRACE("[BattleSystem] Not enough energy ({}/{}) for {}", energy.current, fx->cost, fx->name);
        return;
    }
    ...
    if (preferred_target != entt::null &&
        m_reg.valid(preferred_target) &&
        !m_reg.all_of<DeadTag>(preferred_target))
    {
        target = preferred_target;
    } else {
        for (auto e : m_reg.view<EnemyTag>(entt::exclude<DeadTag>)) { target = e; break; }
    }

    applyCardEffect(*fx, target);

    auto& deck = m_reg.get<DeckComp>(m_player);
    if (!fx->exhaust) deck.discard_pile.push_back(card_id);
...
void BattleSystem::playerEndTurn()
{
    ...
    for (auto e : m_reg.view<EnemyTag>(entt::exclude<DeadTag>)) {
        ...
        if (info) EnemyAI::resolveIntent(m_reg, e, *info);
    }
...
{
    int32_t dmg = base;
    if (auto* s  = m_reg.try_get<StrengthComp>(attacker))  dmg += s->value;
    if (auto* st = m_reg.try_get<StatusComp>(attacker))    if (st->weak > 0)       dmg = dmg * 3 / 4;
    if (auto* st = m_reg.try_get<StatusComp>(target))      if (st->vulnerable > 0) dmg = dmg * 3 / 2;
...
void BattleSystem::applyDamage(entt::entity target, int32_t amount, entt::entity source)
{
    ...
    if (bl) bl->amount -= block_abs;
    int32_t actual = amount - block_abs;
    hp->current   -= actual;

    m_dispatcher.enqueue<EvDamageDealt>({ source, target, amount, actual, block_abs });
    if (hp->isDead()) m_reg.emplace_or_replace<DeadTag>(target);
}

void BattleSystem::applyBlock(entt::entity target, int32_t amount)
{
    int32_t actual = amount;
    if (auto* dex = m_reg.try_get<DexterityComp>(target)) actual += dex->value;
    if (auto* st  = m_reg.try_get<StatusComp>(target))    if (st->frail > 0) actual = actual * 3 / 4;
    actual = std::max(0, actual);
    if (auto* bl = m_reg.try_get<BlockComp>(target)) bl->amount += actual;
...
void BattleSystem::applyCardEffect(const CardInfo& fx, entt::entity target)
{
    ...
    if (fx.heal > 0) {
        ...
        if (hp) hp->current = std::min(hp->current + fx.heal, hp->max);
    }
        ...
        if (auto* st = m_reg.try_get<StatusComp>(target)) {
            ...
            if (fx.apply_weak)
                st->weak = static_cast<int8_t>(
                    std::clamp<int32_t>(st->weak + fx.apply_weak, 0, 127));
        }
    }
}
