...
TurnManager::TurnManager(entt::dispatcher& dispatcher) noexcept
...
void TurnManager::buildEnemyOrder(entt::registry& reg)
{
    ...
    std::sort(m_enemy_order.begin(), m_enemy_order.end(),
              [&](entt::entity a, entt::entity b) {
                  return reg.get<TurnOrderComp>(a).priority <
                         reg.get<TurnOrderComp>(b).priority;
              });
}
...
void TurnManager::endEnemyTurn(entt::registry& reg, CombatContext& ctx,
                                entt::entity enemy)
{
    if (enemy != entt::null)
...
void TurnManager::applyEndOfTurn(entt::registry& reg, entt::entity entity)
{
    ...
    if (st->poison > 0) {
        ...
        --st->poison;
        if (hp.isDead()) reg.emplace_or_replace<DeadTag>(entity);
    }
    ...
    if (st->vulnerable > 0) --st->vulnerable;
    if (st->weak       > 0) --st->weak;
    if (st->frail      > 0) --st->frail;
}
